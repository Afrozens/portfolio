...
const Header = () => {
  ...
  return (
    <motion.header className="h-32 w-full fixed flex items-center justify-between sm:bg-transparent sm:h-28 px-8 z-[99]"
    initial={{ opacity: 0, scale: 0.5 }}
    animate={{ opacity: 1, scale: 1 }}
    transition={{ duration: 0.5, delay: 0.5 }}
    >
      <span className="text-6xl z-[99] font-light">J</span>
      {!isOpen ? (
        <button className="z-[99]" onClick={() => setIsOpen(!isOpen)}>
          <img
            src={hamburguerIcon}
            alt="hamburguer icon"
            className="h-14 w-14 cursor-pointer stroke-[#ff7276]"
          />
        </button>
      ) : (
        <button className="z-[99]" onClick={() => setIsOpen(!isOpen)}>
      ...
      {isOpen && (
        <motion.nav
          ...
          variants={variants}
        >
          <motion.ul className="w-full h-36 mb-8 flex flex-col gap-4 font-light text-lg tracking-widest sm:text-sm"
          initial={{scale: 0.5}}
          ...
          <div className="w-full h-36">
            ...
            <motion.ul className="w-full h-36 mb-8 flex flex-col gap-4 font-light text-sm tracking-widest"
            initial={{scale: 0.5}}
            animate={isOpen && {scale: 1}}
            transition={{duration: 0.3}}
            >
